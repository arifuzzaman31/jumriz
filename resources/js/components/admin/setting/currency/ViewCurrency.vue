        ...
        <div class="ibox-content">
          ...
          >
            <table class="table table-bordered">
              ...
              <tbody>
                <tr v-for="(value, index) in currencies.data" :key="value.id ?? index">
                  <td>{{ value.country }}</td>
                  ...
                  <td>
                    <a
                      v-if="value.currency_status === 0"
                      @click.prevent="makeCurrent(value.id)"
                      class="btn btn-success"
                      href="#"
                    >
                      Set as Your Currency
                    </a>
                    <a v-else class="btn btn-primary" href="#">
                      It's Current Currency
                    </a>
                  </td>
                  <td>
                    <a
                    ...
                    <a
                      @click.prevent="deletecurrency(value.id)"
...
const getcurrency = async (page = 1) => {
  ...
  try {
    const response = await axios.get(
      `${base_url}admin/setting/currency-list?page=${page}&keyword=${keyword.value}`
    )
...
// ✅ This function is called when pagination emits 'page-clicked'
  ...
  if (result.isConfirmed) {
    try {
      const res = await axios.get(
        `${base_url}admin/setting/currency/${id}/delete`
      )
...
const makeCurrent = async (id) => {
  ...
  if (result.isConfirmed) {
    try {
      const res = await axios.get(
        `${base_url}admin/setting/current/currency/${id}`
      )
      successMessage(res.data)
      getcurrency()
    } catch (error) {
...
onMounted(() => {
  getcurrency()
  emitter.on('currency-created', getcurrency)
})
