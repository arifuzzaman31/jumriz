...
class PaypalController extends Controller
{
    ...
    public function payWithpaypal(Request $request,$order_id)
    {
	    ...
	    // Set item list
        $ite = ['product_name' => 'discount'];
        $disc = [
            'id' => 10049,
            'product' => (object)$ite,
            'total_selling_price' => - $order_data->coupon_discount
        ];
        $details_data->push((object)$disc);
        // dd($details_data);
        ...
        $details->setSubtotal((float) $subtotal)
                ->setTax($order_data->vat_gst_amount)
                ->setShipping((float) $order_data->shipping_amount);
                
		// Set payment amount
        $total = ($order_data->total_amount + $order_data->shipping_amount + $order_data->vat_gst_amount) - $order_data->coupon_discount;
		$amount = new Amount();
        ...
        try {
            ...
            if (\Config::get('app.debug')) {
                ...
                return redirect()->route('order.completed',[
                    ...
                    'flug'   => 1,
                    'status' => 'error',
                    'message' => 'Some error occured, Sorry for inconvenient Payment Failed for  #'.$order_id.'.It\'s currently on Cash on Delivery, 
                    if you want to pay now,   
                    then CLICK on My Order button.',
                ]);
        ...
        return redirect()->route('order.completed',[
            ...
            'flug'   => 1,
            'status' => 'error',
            'message' => 'Payment Failed for  #'.$order_id.'.It\'s currently taken as Cash on Delivery, 
            if you want to pay now,    
            then CLICK on My Order button.',
        ]);
