...
// getting current currency
        ...
        $post_data['total_amount'] = ($order->total_amount + $order->shipping_amount + $order->vat_gst_amount) - $order->coupon_discount;
...
# CART PARAMETERS
        ...
        $post_data['product_amount'] = ($order->total_amount + $order->shipping_amount + $order->vat_gst_amount) - $order->coupon_discount;
        $post_data['vat'] = "0";
...
# PARSE THE JSON RESPONSE
        ...
        if (isset($sslcz['GatewayPageURL']) && $sslcz['GatewayPageURL'] != "") {
            ...
            return redirect()->route('order.completed',[
                'flug'    => 1,
                'status'  => 'error',
                'message' => 'Payment failed for json parse error, but your order currenty on  Cash on Delivery,
                if You want to Pay CLICK on My Order button',
            ]);
        }
        ...
        if ($request->status == "VALID") {
         ...
         elseif ($request->status == 'FAILED') 
         {
            ...
            $message = 'Your order no: ' . $request->tran_id . ' Payment 
             failed. but your order Cash on Delivery, if you want to pay now, then CLICK on My Order button';
        } 
        else 
        {
    ...
    public function sslCommerzFailed(Request $request)
    {
        ...
        if ($request->status == 'FAILED') {
            ...
            $message = 'Payment failed for order #'.$request->tran_id.' due to ' . $request->error. ' but order has been taken as 
            Cash on Delivery, if You want to pay now, then CLICK on My Order button';

        } 
        else {

            $message = 'Something went wrong and your payment failed  for order #'.$request->tran_id.', but order has been taken as 
            Cash on Delivery, if you want to pay CLICK on My Order button';
        }
    ...
    public function sslCommerzCancel(Request $request)
    {
        ...
        return redirect()->route('order.completed',[
            ...
            'flug'   => 1,
            'status' => 'error',
            'message' => 'The payment has been canceled for order #'.$request->tran_id.'.but order has been taken as 
            Cash on Delivery if You want to pay, then CLICK on My Order button',
        ]);
