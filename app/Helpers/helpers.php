...
function storeBase64Image($base64Data, $folder)
{
    if (!$base64Data) {
        return null;
    }

    $ext = explode('/', explode(':', substr($base64Data, 0, strpos($base64Data, ';')))[1])[1];
    $fileName = uniqid() . '.' . $ext;
    $relativePath = ltrim($folder, '/') . '/' . $fileName;
    $fileContent = base64_decode(preg_replace('#^data:image/\w+;base64,#i', '', $base64Data));

    Storage::disk('images')->put($relativePath, $fileContent);
...
{
    return App\Models\Setting\ShippingCost::first();
}
...
function allPages()
{
    return cache()->remember('static-page', 43000, function () {
        return App\Models\Setting\PageSetting::where('publish', 1)->get();
    });
}
...
function facebookChat()
{
    return cache()->remember('facebook-setting', 43000, function () {
        return App\Models\Setting\Messenger::orderBy('id', 'desc')->first();
    });
}
...
function googleAnalytics()
{
    return cache()->remember('google-setting', 43000, function () {
        return App\Models\Setting\GoogleAnalytic::orderBy('id', 'desc')->first();
    });
}
...
function frontCategory()
{
    ...
    return cache()->remember('all-category', 43000, function () {
        return App\Models\Category::select('id', 'category_name', 'category_native_name', 'icon')
...
function frontCampaign()
{
    ...
    return App\Models\Campaign::select('id', 'title')->where('status', '=', 1)->get();
...
function getCurrentCurrency()
{
    return cache()->remember('currency', 43000, function () {
        ...
        return App\Models\Currency::where('currency_status', 1)->first();
    });
}

function getLocationData()
{
    return App\Models\Setting\ShippingArea::where('status', 1)->get();
...
function homeSlider()
{
    return cache()->remember('home-slider', 43000, function () {
        return Slider::where('status', '=', AllStatic::$active)->orderBy('id', 'desc')->get();
    });
}
...
function verifyCustomer()
{
    ...
    //     CURLOPT_URL            => "http://track.jumriz.com/api/verify",
